...
const createBooking = (req, res) => {
    const { userId, restaurantId, adults, children, price, date, startTime, endTime } = req.body;
  
    // Check if all required fields are provided
    if (!userId || !restaurantId || !adults || !children || !price || !date || !startTime || !endTime) {
      ...
      if (error) {
        ...
        if (data.length > 0) {
          ...
          for (let i = 0; i < capacity.length; i++) {
            if (capacity[i].tableCapacity >= totalPeople && capacity[i].tableCount > 0) {
          ...
          const updateCapacitySql = "UPDATE restaurant SET capacity = ? WHERE id = ?";
          db.query(updateCapacitySql, [JSON.stringify(capacity), restaurantId], (updateError) => {
            if (updateError) {
            ...
            db.query(sql, [id, userId, restaurantId, 'PENDING', adults, children, price, date, startTime, endTime], (error, result) => {
              if (error) {
                console.error("Error executing query:", error);
                return res.status(500).json({ message: 'Internal Server Error' });
              } else {
