...
export default function RestaurantDetailView() {
  ...
  const tableRows = [
    ...
    { propiedad: 'Horario', valor: Array.isArray(restaurant?.schedule)
      ? restaurant.schedule.map((item) => `${item.day}`).join(', ') : 'No hay datos' },
    { propiedad: 'Tipo', valor: restaurant?.type || 'No hay datos' },
    {
      propiedad: 'Menú',
      valor: Array.isArray(restaurant?.menu_info)
        ? restaurant.menu_info.map(item => `${item.name}: ${item.description} - $${item.price}`).join(', ')
        : 'No hay datos',
    },
    {
      propiedad: 'Reservaciones',
      valor: Array.isArray(restaurant?.reservations)
          ...
          <div className="flex flex-col p-10 gap-10 container">
            <div className="flex flex-col">
              <div className="flex items-center justify-between">
                ...
                <Link href={`/booking/${restaurantId}`} className='btn-primary hover:scale-105 cursor-pointer'>
                  Reservar
                </Link>
              </div>
              <div
